...
////////////////////////////////Optimized for signal analysis/////////////////////

int delphes_CMS13TeV_HHMET_Benchmark(char* string){

  TString finname(string);
  TString foutname(string);
  
  finname = "../../delphes_results/"+finname+"/delphes/myprocess_atlas13tev.root"; // Path to data files
...
//  TLorentzVector b_vector[4];
  ...
  const Int_t     kMaxJetAK8Trim = 50;
  Int_t           nJetAK8Trim = -1;
  Float_t         JetAK8Trim_PT[kMaxJetAK8Trim];   //[JetAK8Trim_]
  Float_t         JetAK8Trim_Eta[kMaxJetAK8Trim];   //[JetAK8Trim_]
  Float_t         JetAK8Trim_Phi[kMaxJetAK8Trim];   //[JetAK8Trim_]
  Float_t         JetAK8Trim_Mass[kMaxJetAK8Trim];   //[JetAK8Trim_]
  TBranch        *b_JetAK8Trim_PT;   //!
  TBranch        *b_JetAK8Trim_Eta;   //!
  TBranch        *b_JetAK8Trim_Phi;   //!
  TBranch        *b_JetAK8Trim_Mass;   //!
  t->SetBranchAddress("JetAK8Trim", &nJetAK8Trim);
  t->SetBranchAddress("JetAK8Trim.PT", JetAK8Trim_PT, &b_JetAK8Trim_PT);
  t->SetBranchAddress("JetAK8Trim.Eta", JetAK8Trim_Eta, &b_JetAK8Trim_Eta);
  t->SetBranchAddress("JetAK8Trim.Phi", JetAK8Trim_Phi, &b_JetAK8Trim_Phi);

  t->SetBranchAddress("JetAK8Trim.Mass", JetAK8Trim_Mass, &b_JetAK8Trim_Mass);
/*
...
//    t->GetEntry(i);
    ...
    jet_m->GetEntry(i);
    jet_btag->GetEntry(i);
    b_JetAK8Trim_PT->GetEntry(i);
    b_JetAK8Trim_Eta->GetEntry(i);
    b_JetAK8Trim_Phi->GetEntry(i);
    b_JetAK8Trim_Mass->GetEntry(i);
...
//////////////////////////////////////////////////////////////////////////////////
////                          Boosted analysis                                ////
////                      based on CMS-SUS-17-006                             ////
//////////////////////////////////////////////////////////////////////////////////

//-------------------------large-R jet(R=1.0)>=2---------------------------
    Int_t Nb_T=0;
    vector< int > fat_index;
    if(nJetAK8Trim<2) continue;
    for(int j=0;j<nJetAK8Trim ;j++){
      if(JetAK8Trim_PT[j]>=250 && fabs(JetAK8Trim_Eta[j])<=2.0 && JetAK8Trim_Mass[j]>=50){
        Double_t pt_leading = 0;
	if(JetAK8Trim_PT[j]>pt_leading) pt_leading=JetAK8Trim_PT[j];
        if(pt_leading>=450){
...
//    if(JetAK8Trim_PT[0]<450 || fabs(JetAK8Trim_Eta[0])>2.0 || JetAK8Trim_Mass[0]<50) continue;
//    if(JetAK8Trim_PT[1]<250 || fabs(JetAK8Trim_Eta[1])>2.0 || JetAK8Trim_Mass[1]<50) continue;
    if(fat_index.size()<2) continue;
    cut_flow[1]++;     // cut: Nb_T>=2

    TLorentzVector h_vector[2];
    h_vector[0].SetPtEtaPhiM(JetAK8Trim_PT[fat_index[0]],JetAK8Trim_Eta[fat_index[0]],JetAK8Trim_Phi[fat_index[0]],JetAK8Trim_Mass[fat_index[0]]);
    h_vector[1].SetPtEtaPhiM(JetAK8Trim_PT[fat_index[1]],JetAK8Trim_Eta[fat_index[1]],JetAK8Trim_Phi[fat_index[1]],JetAK8Trim_Mass[fat_index[1]]);
