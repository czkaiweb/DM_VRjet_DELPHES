...
for i in range(len(MY_l)):
	for j in range(len(MF_l)):
		...
		for k in range(len(g_coupling))	:
			...
			fout.write("#! /bin/bash\n")
			fout.write("python /home/weikai/work/calchep/delphes/prepare_delphes.py %s %s %s %s\n"%(dataname, str(g_coupling[k]),str(int(MY_l[i])),str(int(MF_l[j]))))
			fout.write("/home/weikai/work/tools/CheckMATE-2.0.11/bin/./CheckMATE /home/weikai/work/calchep/delphes/delphes_cards/"+cardname+'-lhe-pythia8-delphes-default.dat\n')
		fout.close()
